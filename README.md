...
- [`make-landing-page`](https://github.com/dgaidula/make-landing-page) — a config-driven scaffolder for placeholder landing pages: correct head, security headers scoped to what the config turns on, robots.txt, sitemap, favicon set, social card, one local commit — and two regions fenced off with `<!-- DESIGN SLOT -->` comments where the real design pass happens, by a human or an agent. Plumbing, not design; the head is populated only from fields the config actually sets, nothing invented. Node, MIT.
